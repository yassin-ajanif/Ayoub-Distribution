...
public partial class BonSortieListViewModel : BaseViewModel
{
    ...
    {
        BtnNew = _locale.T("Btn_NewBonSortie");
        BtnPdf = _locale.T("Btn_Pdf");
        UpdateBtnFilterDateText();
        WmFilterPayee = _locale.T("Bs_FilterPayee");
        LblPayeeFilterAll = _locale.T("Bs_FilterAll");
        LblPayeeFilterUnpaid = _locale.T("Bs_Unpaid");
        LblPayeeFilterPaid = _locale.T("Bs_Paid");
        ...
        ColHeaderPayee = _locale.T("BsList_ColPayee");
    ...
    public ObservableCollection<BonSortieListRow> Items { get; } = [];
    [ObservableProperty] private BonSortieListRow? _selected;
    ...
    private async Task LoadPageAsync(CancellationToken ct, bool resetPage = false)
    {
        ...
        try
        {
            ...
            var q = db.BonsSortie.AsNoTracking().Include(f => f.Lignes).AsQueryable();
            ...
            var selId = Selected?.BonSortie.Id;
            Items.Clear();
            foreach (var f in list)
                Items.Add(BonSortieListRow.Create(f, noms.GetValueOrDefault(f.ClientId) ?? string.Empty, devise, _locale));
            Pagination.TotalCount = total;
            if (selId is { } id)
                Selected = Items.FirstOrDefault(x => x.BonSortie.Id == id);
        }
        finally
        {
            IsBusy = false;
    ...
    private void OpenSelected()
    {
        ...
        var vm = _sp.GetRequiredService<BonSortieEditViewModel>();
        vm.Load(Selected.BonSortie.Id);
        _workspace.Open(vm);
    }

    [RelayCommand]
    private async Task DeleteBonSortieAsync(BonSortieListRow? row, CancellationToken cancellationToken)
    {
        if (row == null) return;
        var item = row.BonSortie;

        if (!await _dialog.ConfirmAsync(_locale.T("Bs_Title"), _locale.Tf("Bs_ConfirmDelete", item.Numero), cancellationToken))
            return;
        ...
        try
        {
            ...
            var entity = await db.BonsSortie.Include(f => f.Lignes).Include(f => f.Paiements).FirstAsync(f => f.Id == item.Id, cancellationToken);
            ...
            if (Selected?.BonSortie.Id == item.Id)
                Selected = null;
            Items.Remove(row);
    ...
    private async Task ExportPdfAsync(CancellationToken cancellationToken)
    {
        ...
        {
            await using var db = await _dbFactory.CreateDbContextAsync(cancellationToken);
            var f = await db.BonsSortie.Include(x => x.Lignes).Include(x => x.Paiements).FirstAsync(x => x.Id == Selected.BonSortie.Id, cancellationToken);
            var client = await db.Tiers.AsNoTracking().FirstAsync(t => t.Id == f.ClientId, cancellationToken);
            var bytes = await _pdf.BuildBonSortiePdfAsync(f, DocumentPartyPdfInfo.FromTiers(client), cancellationToken);
