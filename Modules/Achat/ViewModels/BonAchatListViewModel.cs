...
public partial class BonAchatListViewModel : BaseViewModel
{
    ...
    {
        BtnNew = _locale.T("Btn_NewBonAchat");
        BtnPdf = _locale.T("Btn_Pdf");
        UpdateBtnFilterDateText();
        WmFilterPayee = _locale.T("Ba_FilterPayee");
        LblPayeeFilterAll = _locale.T("Ba_FilterAll");
        LblPayeeFilterUnpaid = _locale.T("Ba_Unpaid");
        LblPayeeFilterPaid = _locale.T("Ba_Paid");
        ...
        ColHeaderPayee = _locale.T("BaList_ColPayee");
    ...
    public ObservableCollection<BonAchatListRow> Items { get; } = [];
    [ObservableProperty] private BonAchatListRow? _selected;
    ...
    private async Task LoadPageAsync(CancellationToken ct, bool resetPage = false)
    {
        ...
        try
        {
            ...
            var q = db.BonsAchat.AsNoTracking().Include(f => f.Lignes).AsQueryable();
            ...
            var selId = Selected?.BonAchat.Id;
            Items.Clear();
            foreach (var f in list)
                Items.Add(BonAchatListRow.Create(f, noms.GetValueOrDefault(f.FournisseurId) ?? string.Empty, devise, _locale));
            Pagination.TotalCount = total;
            if (selId is { } id)
                Selected = Items.FirstOrDefault(x => x.BonAchat.Id == id);
        }
        finally
        {
            IsBusy = false;
    ...
    private void OpenSelected()
    {
        ...
        var vm = _sp.GetRequiredService<BonAchatEditViewModel>();
        vm.Load(Selected.BonAchat.Id);
        _workspace.Open(vm);
    }

    [RelayCommand]
    private async Task DeleteBonAchatAsync(BonAchatListRow? row, CancellationToken cancellationToken)
    {
        if (row == null) return;
        var item = row.BonAchat;

        if (!await _dialog.ConfirmAsync(_locale.T("Ba_Title"), _locale.Tf("Ba_ConfirmDelete", item.Numero), cancellationToken))
            return;
        ...
        try
        {
            ...
            var entity = await db.BonsAchat.Include(f => f.Lignes).Include(f => f.Paiements).FirstAsync(f => f.Id == item.Id, cancellationToken);
            ...
            if (Selected?.BonAchat.Id == item.Id)
                Selected = null;
            Items.Remove(row);
    ...
    private async Task ExportPdfAsync(CancellationToken cancellationToken)
    {
        ...
        {
            await using var db = await _dbFactory.CreateDbContextAsync(cancellationToken);
            var f = await db.BonsAchat.Include(x => x.Lignes).Include(x => x.Paiements).FirstAsync(x => x.Id == Selected.BonAchat.Id, cancellationToken);
            var fournisseur = await db.Tiers.AsNoTracking().FirstAsync(t => t.Id == f.FournisseurId, cancellationToken);
            var bytes = await _pdf.BuildBonAchatPdfAsync(f, DocumentPartyPdfInfo.FromTiers(fournisseur), cancellationToken);
