...
public static class MouvementStockQueries
{
        ...
        string? searchTerm)
    {
        ...
        return query.Where(m =>
            ...
            || (m.OrigineType == StockMovementService.OrigineTypeBonSortie
                && m.OrigineId != null
                && db.BonsSortie.Any(bp =>
                    bp.Id == m.OrigineId
                    && db.Tiers.Any(tier => tier.Id == bp.ClientId && tier.Nom.ToLower().Contains(t))))
            || (m.OrigineType == StockMovementService.OrigineTypeBonAchat
                && m.OrigineId != null
                && db.BonsAchat.Any(ba =>
                    ba.Id == m.OrigineId
                    && db.Tiers.Any(tier => tier.Id == ba.FournisseurId && tier.Nom.ToLower().Contains(t))))
            || (m.OrigineType == StockMovementService.OrigineTypeBonRetour
                && m.OrigineId != null
                && db.BonsRetour.Any(a =>
                    a.Id == m.OrigineId
