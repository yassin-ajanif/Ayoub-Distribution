...
public partial class App : Application
{
    ...
    public override void OnFrameworkInitializationCompleted()
    {
        if (ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            ...
            // Show the window first so migrate/lock issues don't leave a blank process.
    ...
    /// <summary>Clears a leftover EF migration lock that can leave the app hanging with no UI.</summary>
    private static void ClearMigrationsLock()
    {
        try
        {
            using var con = new SqliteConnection(DatabasePath.GetConnectionString());
            con.Open();
